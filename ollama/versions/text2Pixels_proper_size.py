...
def put_text(frame, text: str, start, character_index, scale=5):
    row_start, column_start = start
    row_spacing = 12  # Adjust as needed
    column_spacing = 8  # Adjust as needed
    cursor = [row_start, column_start]
    max_width = 0
    max_height = 0
    for c in text:
        if c == "\n":
            cursor[0] += row_spacing
            cursor[1] = column_start  # Reset column position for new line
            continue
        ...
        max_width = max(max_width, cursor[1] + scaled_letter_width)
        max_height = max(max_height, cursor[0] + scaled_letter_height)

        # Check if adding the letter exceeds the frame boundaries
        if cursor[0] + scaled_letter_height > frame.shape[0] or cursor[1] + scaled_letter_width > frame.shape[1]:
        ...
        for i in range(scaled_letter_height):
            for j in range(scaled_letter_width):
                frame[cursor[0] + i][cursor[1] + j] = letter_data[i // scale][j // scale]

        # Update cursor position for the next character
        cursor[1] += scaled_letter_width + column_spacing
