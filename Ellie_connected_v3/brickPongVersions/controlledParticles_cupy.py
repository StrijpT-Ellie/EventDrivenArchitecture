...
class Particle:
    def __init__(self, x, y, speed, direction, color):
        ...
        self.ax = random.uniform(-0.1, 0.1)
        self.ay = random.uniform(-0.1, 0.1)
        self.damping = 0.99
    ...
    def update_params(self, speed, direction):
        self.speed = speed
        self.direction = direction
        self.dx = speed * cp.cos(cp.radians(direction))
        self.dy = speed * cp.sin(cp.radians(direction))
...
class ParticleEmitter:
    ...
    def check_collision(self, particle1, particle2):
        distance = cp.sqrt((particle1.x - particle2.x)**2 + (particle1.y - particle2.y)**2)
        return distance < 12
...
class Game:
    ...
    def run(self):
        ...
        while running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        emitter = ParticleEmitter(self.width, self.height, 25, self.particles[0])
                        emitter.trigger()
                    elif event.key == pygame.K_b:
                        block_emitter.trigger(25)

            if self.mode == 'autopilot':
                for particle in self.particles:
                    particle.update_acceleration(random.uniform(-0.01, 0.01), random.uniform(-0.01, 0.01))
                ...
                for particle in self.particles:
                    if direction == 'up':
                        particle.update_acceleration(0.5, -0.5)
                    ...
                    particle.move(self.width, self.height)
                    particle.update_acceleration(0, 0)

                block_emitter.update(self.particles)
                block_emitter.draw(self.screen)

                if emitter:
                    emitter.update(block_emitter.blocks)
            ...
            reduced_color_array = (resized_array / 32).astype(cp.uint8) * 32
            ...
            if frame_counter % N == 0:
                threading.Thread(target=visualize_array, args=(reduced_color_array.get(),)).start()
