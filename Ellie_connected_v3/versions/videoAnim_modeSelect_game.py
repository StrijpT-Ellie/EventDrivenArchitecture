#An attempt to realise event-driven architecture with mode selector 
#switched to videoAnim_modeSelect_game copy.py
...
def apply_floating_effect(positions, motion_detected):
    if motion_detected:
...
def run_pixelated_animation():
    cap = cv2.VideoCapture(0)
    global motion_detected, person_detected_time, start_time, prev_gray
    ...
    while True:
        ...
        apply_floating_effect(pixel_positions, motion_detected)
...
def main():
    # Run the pixelated animation until person detected for 30 seconds
    run_pixelated_animation()
    ...
    if mode == 1:
        modes_text = "art"
        script_name = "art_mode.py"
    elif mode == 2:
        modes_text = "game"
        script_name = "game_mode.py"
    elif mode == 3:
        modes_text = "chat"
        script_name = "chat_mode.py"
    else:
    ...
    if script_name:
        ...
        # Launch the selected mode script
        script_process = subprocess.Popen(['python', script_name])

        # Monitor the .txt file for updates
        def monitor_file():
            ...
            while True:
                with open("coordinates.txt", "r") as f:
                    ...
                    if lines:
                        ...
                        try:
                            timestamp = float(last_line.split()[-1])
