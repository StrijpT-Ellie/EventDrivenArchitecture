...
class Game:
    ...
    def run(self):
        ...
        running = True
        emitter = None
        block_emitter = BlockEmitter(self.width, self.height, 50, 12)
        frame_counter = 0
        N = 32
                ...
                if event.type == pygame.QUIT:
                 ...
                 #   if event.key == pygame.K_SPACE:  # Press space to switch modes
                  #      self.mode = 'manual' if self.mode == 'autopilot' else 'autopilot'
                    ...
                    if event.key == pygame.K_p:  # Press "p" to trigger the emitter
                        ...
                        block_emitter.trigger(60)  # Emit 100 blocks

            # Monitor the text file for changes and trigger action if "person" is detected
            try:
                with open('labels.txt', 'r') as f:
                    lines = f.readlines()
                    if lines:  # Check if the list is not empty
                        last_line = lines[-1]
                        if 'person' in last_line:
                            # Trigger your action here
                            print("Person detected! Triggering action...")
                            # Reset the text file after triggering action
                            open('labels.txt', 'w').close()

                            time.sleep(1)  # Add a delay to prevent repeated triggering
                            # Switch mode to manual
                            try:
                                self.mode = 'manual'
                            except AttributeError:
                                print("Error: 'self.mode' is not defined or not allowed to be changed.")
            except IndexError:
...
if __name__ == "__main__":
    ...
    #run 
    #  py -3.11 .\contourWallAI_blocksNRows_eventDriven.py