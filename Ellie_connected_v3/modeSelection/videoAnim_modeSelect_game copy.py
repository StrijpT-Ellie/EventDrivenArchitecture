#This script launches animation and detects a person 
#It launches the mode selection window when a user is present for 30 sec 
#then it waits for user input as fingers for gesture recognition 
#based on finger number it launches a mode (game mode only now) 
#then if user doesn't move a bar for 30 sec it closes the game and relaunches the animation 
#However, after the animation is relaunched for the second time it doesn't open the mode selection, but onlyh restarts in a loop
...
class EventHandler:
    def __init__(self, animation, mode_selector):
