...
class ParticleEmitter:
    ...
    def draw(self, screen):
        for particle in self.particles:
            pygame.draw.circle(screen, particle.color, (int(particle.x), int(particle.y)), 10)
        ...
        for block in self.blocks[:]:
            for particle in particles:
                if block.x < particle.x < block.x + block.width and block.y < particle.y < block.y + block.height:
                    blocks_to_remove.append(block)
                    particle.dy *= -1
                    particle.dx += random.uniform(-2, 5)
                    break
...
class MovingBar:
    def __init__(self, screen_width, screen_height, bar_width=200, bar_height=10):
        self.screen_width = screen_width
        self.screen_height = screen_height
...
class Game:
    def __init__(self, width, height, grid_size, direction, num_particles, speed):
        ...
        self.frame_rate = 20  # Lower frame rate to reduce CPU load
        self.last_print_time = time.time()
        self.print_interval = 1.0

        self.main_particle = Particle(width // 2, height // 2, speed * 2, direction, (255, 255, 255))
        self.emitter = ParticleEmitter(self.width, self.height, 10, self.main_particle)  # Reduce number of particles
    ...
    def run(self):
        running = True
        block_emitter = BlockEmitter(self.width, self.height, 40, 8)
        block_emitter.trigger(32, 40, 20)  # Reduce number of blocks
        frame_counter = 0
        N = 32
        ...
        while running:
            ...
            for particle in self.particles:
                pygame.draw.circle(self.screen, (255, 255, 255), (int(particle.x), int(particle.y)), 10)
            ...
            for particle in self.particles:
                particle.update_acceleration(0, 0.25)
                ...
                if (self.moving_bar.x < particle.x < self.moving_bar.x + self.moving_bar.bar_width and
                    ...
                    particle.dy *= -2
                    particle.dx += random.uniform(-2, 5)
                    particle.y = self.moving_bar.y - 10
            ...
            if self.destroyed_bricks >= 10 and not self.emitter_triggered:
            ...
            if block_emitter.last_destroyed_time and (time.time() - block_emitter.last_destroyed_time >= 10):
                block_emitter.trigger(32, 40, 20)
