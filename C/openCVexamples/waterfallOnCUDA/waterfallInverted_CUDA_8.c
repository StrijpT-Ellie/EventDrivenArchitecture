...
bool detect_motion(const cv::cuda::GpuMat& frame, const cv::cuda::GpuMat& prev_frame, cv::cuda::Stream &stream);
...
bool detect_motion(const cv::cuda::GpuMat& frame, const cv::cuda::GpuMat& prev_frame, cv::cuda::Stream &stream) {
    cv::cuda::GpuMat frame_diff, threshold_frame, dilated_frame;
    cv::cuda::absdiff(frame, prev_frame, frame_diff, stream);
    cv::cuda::threshold(frame_diff, threshold_frame, 25, 255, cv::THRESH_BINARY, stream);

    cv::Ptr<cv::cuda::Filter> dilate_filter = cv::cuda::createMorphologyFilter(cv::MORPH_DILATE, threshold_frame.type(), Mat(), Point(-1, -1), 2);
    dilate_filter->apply(threshold_frame, dilated_frame, stream);

    cv::Mat h_dilated_frame;
    dilated_frame.download(h_dilated_frame, stream);
    stream.waitForCompletion();

    return cv::sum(h_dilated_frame)[0] > 10000;
}
...
int main(int argc, char** argv) {
    ...
    cv::cuda::GpuMat gpu_frame, gpu_gray, gpu_prev_gray, gpu_blurred;
    ...
    while (true) {
        ...
        motion_detected = detect_motion(gpu_blurred, gpu_prev_gray, stream);

        gpu_prev_gray = gpu_blurred.clone();

        Mat resized_frame, pixelated_frame;
        gpu_frame.download(resized_frame, stream); // Download only for operations not available on GPU
        ...
        gpu_resized_frame.download(resized_frame, stream); // Download after quantization
