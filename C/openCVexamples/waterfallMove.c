//This is a translated version of floatingParticles_stronger.py 

//To compile 
//g++ -std=c++11 -I/opt/homebrew/Cellar/opencv/4.9.0_8.reinstall/include/opencv4 -L/opt/homebrew/Cellar/opencv/4.9.0_8.reinstall/lib -lopencv_core -lopencv_imgproc -lopencv_highgui -lopencv_videoio -o waterfallMove waterfallMove.c
 
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include </opt/homebrew/Cellar/opencv/4.9.0_8.reinstall/include/opencv4/opencv2/opencv.hpp>
...
Vec3b quantize_color(Vec3b pixel) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < PIXELATED_HEIGHT; ++i) {
        for (int j = 0; j < PIXELATED_WIDTH; ++j) {
            original_positions.push_back(Point(j, i));
        }
    }
    ...
    while (true) {
        ...
        for (int i = 0; i < resized_frame.rows; ++i) {
            for (int j = 0; j < resized_frame.cols; ++j) {
                resized_frame.at<Vec3b>(i, j) = quantize_color(resized_frame.at<Vec3b>(i, j));
            }
        }
        ...
        for (size_t i = 0; i < pixel_positions.size(); ++i) {
            Point pos = pixel_positions[i];
