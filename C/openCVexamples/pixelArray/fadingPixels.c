...
#define FADE_DURATION 90  // Duration for red pixels to fade back to green in frames (approx 3 seconds at 30 FPS)
        ...
        for (int x = 0; x < LED_WIDTH; x++) {
            if (led_states[y][x].timer > 0) {
                ...
                if (led_states[y][x].timer > 0) {
                    ...
                    led_states[y][x].color = Scalar(0, 255 * (1 - ratio), 255 * ratio);
                } else {
