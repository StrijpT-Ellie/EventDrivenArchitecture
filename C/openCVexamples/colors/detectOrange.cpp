...
void detect_and_float_orange_pixels(const Mat &frame, vector<FloatingPixel> &floating_pixels, vector<vector<PixelState>> &pixel_states) {
    Mat hsv_frame;
    ...
    Mat orange_mask;
    inRange(hsv_frame, lower_orange, upper_orange, orange_mask);

    // Update pixel states based on orange detection
    for (int y = 0; y < orange_mask.rows; y++) {
        for (int x = 0; x < orange_mask.cols; x++) {
            bool isOrange = orange_mask.at<uchar>(y, x) > 0;
    ...
    // Find orange pixels and add them to the floating pixels list
    int new_floating_pixels_count = 0;
    for (int y = 0; y < orange_mask.rows; y++) {
        for (int x = 0; x < orange_mask.cols; x++) {
            if (pixel_states[y][x].isOrange) {
                if (new_floating_pixels_count < MAX_NEW_FLOATING_PIXELS) {
                    FloatingPixel fp;
                    fp.position = Point(x, y);
                    fp.color = Scalar(0, 165, 255); // Orange color
...
void draw_led_wall(Mat &led_wall, const Mat &frame, const vector<FloatingPixel> &floating_pixels, const vector<vector<int>> &accumulated_pixels) {
    ...
    for (int y = 0; y < LED_HEIGHT; y++) {
        for (int x = 0; x < LED_WIDTH; x++) {
            if (accumulated_pixels[y][x] == 1) {
                Rect led_rect(x * (led_size_x + LED_SPACING), y * (led_size_y + LED_SPACING), led_size_x, led_size_y);
                rectangle(led_wall, led_rect, Scalar(0, 165, 255), FILLED);
            }
        }
    }
}
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        // Detect orange pixels and add floating effect
        detect_and_float_orange_pixels(avg_frame, floating_pixels, pixel_states);
