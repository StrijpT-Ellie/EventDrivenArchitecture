...
#define FRAME_RATE 30  // Reduce frame rate to lower CPU load
...
void initialize_bar(Bar &bar) {
    bar.position = Point2f(DISPLAY_WIDTH / 2, DISPLAY_HEIGHT - BAR_HEIGHT);
    bar.width = 100;
...
void draw_led_wall(Mat &led_wall, const FloatingBall &ball, const Bar &bar, const vector<Brick> &bricks) {
    ...
    circle(led_wall, ball.position, ball.radius, ball.color, FILLED);
