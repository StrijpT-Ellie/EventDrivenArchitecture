...
def objective_func(params):
    goal = [20 / 2, 0]  # Middle point of the left
    total_distance = 0
    for i in range(0, len(params), 2):
        speed = params[i]
        direction = params[i + 1]
        # Calculate new position after moving
        x = speed * math.cos(math.radians(direction))
        y = speed * math.sin(math.radians(direction))
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_SPACE:  # Press space to switch mode
                        ...
                        emitter = ParticleEmitter(self.width, self.height, 100)
                        emitter.trigger()
