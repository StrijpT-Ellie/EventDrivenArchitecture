...
# Function to apply the falling sand effect
def apply_falling_sand(pixels):
    for y in range(pixelated_height - 2, -1, -1):
        for x in range(pixelated_width):
            if np.any(pixels[y, x]) and not np.any(pixels[y + 1, x]):
                pixels[y + 1, x] = pixels[y, x]
                pixels[y, x] = [0, 0, 0]

    # Apply fade effect to all pixels
    fade_factor = 0.9  # Adjust fade factor to control fading speed
    pixels[:] = cv2.addWeighted(pixels, fade_factor, np.zeros_like(pixels), 1 - fade_factor, 0)

    # Clear fully faded pixels
    threshold = 5  # Threshold to determine when a pixel is considered faded out
    mask = np.all(pixels < threshold, axis=-1)
    pixels[mask] = [0, 0, 0]

def calculate_brush_size(z_value):
    # Normalize Z-value to a range suitable for brush size
    min_z = 0.0000001  # Set a realistic minimum Z-value
    max_z = 0.0000005  # Set a realistic maximum Z-value
    z_normalized = (z_value - min_z) / (max_z - min_z)
    z_normalized = np.clip(z_normalized, 0, 1)  # Ensure the normalized value is between 0 and 1
    brush_size = int(1 + (z_normalized * 6))  # Brush size between 1 and 6
    return brush_size
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # Calculate brush size based on the Z-coordinate of the palm base
            z_value = palm_base.z
            print(f"Z-value: {z_value}")  # Debug statement to see Z-values
            brush_size = calculate_brush_size(z_value)
            print(f"Brush size: {brush_size}")  # Debug statement to see calculated brush sizes
    ...
    # Apply the falling sand effect to persistent pixels
